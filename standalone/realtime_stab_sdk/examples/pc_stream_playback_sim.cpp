...
bool probe_video(const std::string& input, int& w, int& h, double& fps) {
    ...
    const size_t slash = line_rate.find('/');
...
int main(int argc, char** argv) {
    if (argc < 3) {
        std::cerr << "Usage: pc_stream_playback_sim <input_video> <output_video>" << std::endl;
        return 1;
    }
    ...
    const int latency_frames = std::max(1, static_cast<int>(std::lround(0.5 * fps))); // ~0.5s

    rtsdk::StabilizerConfig cfg;
    cfg.width = width;
    cfg.height = height;
    cfg.input_channels = 3;
    cfg.motion_estimator = 1;
    cfg.smoothing_mode = 1;
    cfg.motion_model = 1;
    cfg.trim_ratio = 0.04f;
    cfg.latency_radius = latency_frames;
    ...
    std::deque<std::vector<uint8_t> > playback_queue;
    const size_t playback_start_threshold = static_cast<size_t>(latency_frames);

    auto emit_if_ready = [&]() -> bool {
        while (playback_queue.size() > playback_start_threshold) {
            std::vector<uint8_t>& frame = playback_queue.front();
            if (std::fwrite(frame.data(), 1, frame_bytes, encoder) != frame_bytes) {
                return false;
            }
    ...
    while (std::fread(in.data(), 1, frame_bytes, decoder) == frame_bytes) {
        ...
        playback_queue.emplace_back(out);
        if (!emit_if_ready()) {
    ...
    while (!playback_queue.empty()) {
        std::vector<uint8_t>& frame = playback_queue.front();
        if (std::fwrite(frame.data(), 1, frame_bytes, encoder) != frame_bytes) {
            std::cerr << "Write tail frame failed." << std::endl;
            PCLOSE(decoder);
            PCLOSE(encoder);
            return 7;
        }
